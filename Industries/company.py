...
#'''
#获取各地区链接
#r = requests.get(r"https://shanghai.11467.com/yangpu/",headers=headers,cookies=cookies)
#html = r.text
#soup = BeautifulSoup(html)
#print(soup)
#for tag in soup.find(name='div', attrs={"class":"f_l"}):
#    print(tag.string)
#    with open(r"H:\urban renewal\企业\company.csv","a",encoding="UTF-8") as csvfile:
#            writer = csv.writer(csvfile)
#            writer.writerow([tag.string])

base_url = r"https://shanghai.11467.com/yangpu/pn"
for i in range(1,21):
    url = base_url + str(i)
    r = requests.get(url,headers=headers,cookies=cookies)
    html = r.text
    soup = BeautifulSoup(html)
    for tag in soup.find(name='div', class_="f_l").find_all('h4'):
        #print(tag)
        with open(r"H:\urban renewal\企业\company.csv","a",encoding="UTF-8") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([tag.string])
